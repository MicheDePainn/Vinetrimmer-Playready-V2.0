        ...
        def process_track_decryption(track):
            ...
            if track.encrypted:
                ...
                if not track.key:
                    ...
                    with cdm_lock:
                        # Re-check cache inside lock in case another thread just filled it
                        if not no_cache:
                            track.key, _ = ctx.obj.vaults.get(track.kid, title.id)
                        ...
                        if not track.key:
                            ...
                            for vault in ctx.obj.vaults.vaults:
                                ...
                                for kid, key in content_keys:
                                    ...
                                    if result == InsertResult.SUCCESS: cached += 1
                                    elif result == InsertResult.ALREADY_EXISTS: already_exists += 1
                            ...
                            log.info(f"Decrypting {track.id}...")
                            log.info(f" + KEY: {track.key} (Shared from parallel track)")
